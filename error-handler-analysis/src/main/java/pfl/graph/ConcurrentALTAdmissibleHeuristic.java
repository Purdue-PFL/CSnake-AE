 ...
 */
public class ConcurrentALTAdmissibleHeuristic<V, E>
    implements AStarAdmissibleHeuristic<V>
{
     ...
     */
    public ConcurrentALTAdmissibleHeuristic(Graph<V, E> graph, Set<V> landmarks)
    {
        ...
        this.fromLandmark = new ConcurrentHashMap<>();
        if (graph.getType().isDirected()) {
            this.directed = true;
            this.toLandmark = new ConcurrentHashMap<>();
    ...
    private void precomputeToFromLandmark(V landmark)
    {
        ...
        Map<V, Double> fromLandMarkDistances = new ConcurrentHashMap<>();
        ...
        if (directed) {
            ...
            Map<V, Double> toLandMarkDistances = new ConcurrentHashMap<>();
            for (V v : graph.vertexSet()) {
